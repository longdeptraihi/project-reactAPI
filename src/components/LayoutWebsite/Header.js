import React from 'react'

const Header = () => {
    return (
        <div>
            <div className="header-top ">
                <div className="header">
                    <div className="header-left">
                        <div className="header-logo">
                            <img src="./img/Logomobile.png" />
                        </div>
                        <div className="header-menu">
                            <nav className="nav-pc">
                                <ul>
                                    <li><a href="" />home</li>
                                    <li><a href="" />shop</li>
                                    <li><a href="" />hihi</li>
                                </ul>
                            </nav>
                        </div>
                        <div className="nav-menu-mobile">
                            <input type="checkbox" name className="nav__input" id="nav-mobile-input" />
                            <nav className="nav-mobile">
                                <label htmlFor="nav-mobile-input" className="nav-mobile-close">
                                    <svg aria-hidden="true" focusable="false" data-prefix="fas" data-icon="times" className="svg-inline--fa fa-times fa-w-11" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 352 512">
                                        <path fill="currentColor" d="M242.72 256l100.07-100.07c12.28-12.28 12.28-32.19 0-44.48l-22.24-22.24c-12.28-12.28-32.19-12.28-44.48 0L176 189.28 75.93 89.21c-12.28-12.28-32.19-12.28-44.48 0L9.21 111.45c-12.28 12.28-12.28 32.19 0 44.48L109.28 256 9.21 356.07c-12.28 12.28-12.28 32.19 0 44.48l22.24 22.24c12.28 12.28 32.2 12.28 44.48 0L176 322.72l100.07 100.07c12.28 12.28 32.2 12.28 44.48 0l22.24-22.24c12.28-12.28 12.28-32.19 0-44.48L242.72 256z">
                                        </path>
                                    </svg>
                                </label>
                                <ul className="nav-mobile-list ">
                                    <li className="d-flex">home</li>
                                    <li className="d-flex">shop</li>
                                    <li className="d-flex">hihi</li>
                                </ul>
                            </nav>
                            <label htmlFor="nav-mobile-input" className="nav-overlay" />
                        </div>
                    </div>
                    <div className="header-right">
                        <div className="header-right__icon">
                            <img src="./img/CartButton.png" />
                        </div>
                        <div className="header-right-login">
                            LOGIN
                        </div>
                    </div>
                    <label htmlFor="nav-mobile-input" className="header-menu-mobile">
                        <svg aria-hidden="true" focusable="false" data-prefix="fas" data-icon="bars" className="svg-inline--fa fa-bars fa-w-14" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512">
                            <path fill="currentColor" d="M16 132h416c8.837 0 16-7.163 16-16V76c0-8.837-7.163-16-16-16H16C7.163 60 0 67.163 0 76v40c0 8.837 7.163 16 16 16zm0 160h416c8.837 0 16-7.163 16-16v-40c0-8.837-7.163-16-16-16H16c-8.837 0-16 7.163-16 16v40c0 8.837 7.163 16 16 16zm0 160h416c8.837 0 16-7.163 16-16v-40c0-8.837-7.163-16-16-16H16c-8.837 0-16 7.163-16 16v40c0 8.837 7.163 16 16 16z">
                            </path>
                        </svg>
                    </label>
                </div>
            </div>
            <div className="header-bot mt-3 row">
                <div className="header__bot-left d-flex justify-content-start col-9">
                    <div className="header__bot-icon">
                        <img src="./img/Search.png" />
                    </div>
                    <div className="header__bot-search d-flex align-items-end ">
                        <input className="form-control" type="text " placeholder="living room" />
                    </div>
                </div>
                <div className="header__bot-right d-flex align-items-end col-3 ">
                    <select className="form-select" aria-label="Default select example">
                        <option selected>Open this select menu</option>
                        <option value={1}>One</option>
                        <option value={2}>Two</option>
                        <option value={3}>Three</option>
                    </select>
                </div>
            </div>
        </div>
    )
}

export default Header
